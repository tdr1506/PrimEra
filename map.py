...
class NetworkAnalyzerApp:
    def __init__(self, root):
        ...
        self.output_tree = ttk.Treeview(self.output_tree_frame, columns=("Time", "Source IP", "Source Port", "Destination IP", "Destination Port", "Protocol", "Location"))
        self.output_tree.heading("#0", text="Packet No.", anchor=tk.CENTER)
        self.output_tree.heading("Time", text="Time", anchor=tk.CENTER)
        ...
        self.remedial_measures_frame.grid(row=2, column=0, padx=10, pady=10, sticky="nsew")
    ...
    def analyze_pcap(self):
        ...
        for idx, pkt in enumerate(pkts, start=1):
            ...
            if ip_layer:
                ...
                results.append([time, src_ip, src_port, dst_ip, dst_port, protocol, length, location])
    ...
    def get_top_conversations(self, results):
        conversations = Counter((result[1], result[2], result[3], result[4], result[5]) for result in results)
        top_conversations = "\n".join(f"{k}: {v} packets" for k, v in conversations.most_common(5))
        return top_conversations

    def get_top_locations(self, results):
        locations = Counter(result[7] for result in results if result[7])
